...
| C Live AI and dynamic follow-up | Ready | Pending V2 main-task dispatch | - | Server-only Gateway provider, `AI_EXECUTION_MODE`, operation budgets/routing, redacted model-call telemetry, current-model preflight, and bounded schema-validated dynamic follow-ups that cannot change deterministic calculations. |
