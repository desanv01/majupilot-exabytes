...
vi.mock("@/infrastructure/persistence/api", async (importOriginal) => {
  ...
  return { ...original, resolveOwner: mocks.resolveOwner, repository: () => ({ assertAssessmentAccess: mocks.assertAssessmentAccess }), correlationId: () => "request_test123", response: (body: unknown, status: number) => Response.json(body, { status }) };
});
...
const valid = {
  ...
  answeredIntents: [], automaticFollowUpCount: 0, evidenceRefs: [],
};
...
describe("Phase C follow-up route", () => {
  ...
  it("rejects malformed structured input before ownership/provider work", async () => {
    ...
    const result = await POST(new Request("http://localhost/api/v2/assessment/follow-up", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ ...valid, automaticFollowUpCount: 4 }) }));
