...
- Baseline: `b493f8f`
- Implementation commit: this file's containing commit; the exact SHA is recorded in the final handoff because a Git commit cannot contain its own hash.
...
4. Retry had no stable failed-turn key. The repair reuses the original key, removes the failed optimistic status without adding another user bubble, and stores a safe immutable failure receipt so server replay cannot append duplicate messages or confirmation proposals.
...
- Retry is shown only for retryable turn failures, retains the original idempotency key, avoids a second optimistic user message, and replays either the durable success or safe failure receipt.
...
- The required-mode timeout test snapshots deterministic evidence, confirms no read or write tool executes, replays the same failed-turn receipt, and proves only one user message and one receipt exist.
