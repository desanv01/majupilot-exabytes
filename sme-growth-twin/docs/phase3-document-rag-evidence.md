...
- Metadata lifecycle with checksum duplicate detection, failed reprocessing,
  conditional lifecycle transitions, retryable deletion cleanup, and 60-second
  signed downloads. Browser roles have no direct evidence Storage policy.
...
| Gate | Result |
| --- | --- |
| Full Vitest suite | 243 passed; 2 intentionally skipped (existing opt-in test plus new local DB test) |
...
| Real local Storage + pgvector | Opt-in integration test passed separately |
...
| Repository secret scan | 613 tracked text files; 0 credential values or client model-provider imports |
| Production UI browser harness | Passed: upload, duplicate, unsupported, oversize, failed/reprocess, delete, visible Copilot citations |
...
- Live Gateway embeddings and live model prompt-injection behavior were not
  exercised: this worktree had no Gateway credentials. The fixed model was
  verified against the live catalogue; local provider isolation and tool bounds
