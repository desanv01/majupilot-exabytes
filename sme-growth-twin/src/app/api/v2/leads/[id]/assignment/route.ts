...
const querySchema = z.object({ organizationId: z.uuid().optional() }).strict();
...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return response({ data: await new DurableLeadService(new SupabaseDurableLeadRepository()).assignment(access, leadId) }, 200, requestId);
