...
function BlueprintReport({ blueprint }: { blueprint: Blueprint }) {
  ...
  return <div className="blueprint-layout"><aside className="contents-rail no-print"><strong>Blueprint contents</strong><nav aria-label="Blueprint sections">{sections.map(([anchor, label], index) => <a href={`#${anchor}`} key={anchor}><span>{String(index + 1).padStart(2, "0")}</span>{label}</a>)}</nav></aside><article className="blueprint-report">
    ...
    <section id="consultation-preview"><p className="section-number">15</p><h2>Consultation handoff</h2><p>Request a consultation using this exact, immutable Blueprint. You will review what is shared, add contact details, and give explicit consent before any request is recorded.</p><div className="report-consultation-action no-print"><Link className="button primary" href="/consultation">Request consultation</Link><small>The prototype records the request process-locally and does not send email or CRM notifications.</small></div></section>
