...
import { CONSENT_WORDING, CONSENT_WORDING_VERSION, contactSchema, leadReceiptSchema, type LeadReceipt } from "@/domain/leads";
...
export function ConsultationView({ blueprint, initialReceipt }: { blueprint: Blueprint; initialReceipt?: LeadReceipt }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      const response = await fetch("/api/leads", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ submissionId: submissionId.current, contact: checked.contact, consent: { accepted: form.consent, wordingVersion: CONSENT_WORDING_VERSION }, honeypot: form.website, blueprint }) });
  ...
  return <main className="consultation-shell">
    ...
    <div className="consultation-grid"><form className="consultation-form" onSubmit={submit} noValidate>
      <section><h2>Contact details</h2><p>Tell us who a future consultant should contact about this Blueprint.</p><div className="consultation-fields">
        <label><span>Contact name <b aria-hidden="true">*</b></span><input name="name" autoComplete="name" value={form.name} onChange={(e) => update("name", e.target.value)} aria-invalid={Boolean(errors.name)} aria-describedby={errors.name ? "name-error" : undefined} />{errors.name ? <small className="field-error" id="name-error">{errors.name}</small> : null}</label>
        <label><span>Business name <b aria-hidden="true">*</b></span><input name="businessName" autoComplete="organization" value={form.businessName} onChange={(e) => update("businessName", e.target.value)} aria-invalid={Boolean(errors.businessName)} aria-describedby={errors.businessName ? "business-error" : undefined} />{errors.businessName ? <small className="field-error" id="business-error">{errors.businessName}</small> : null}</label>
        <label><span>Email address <b aria-hidden="true">*</b></span><input name="email" type="email" autoComplete="email" inputMode="email" value={form.email} onChange={(e) => update("email", e.target.value)} aria-invalid={Boolean(errors.email)} aria-describedby={errors.email ? "email-error" : "email-hint"} /><small id="email-hint">Used only for this consultation request.</small>{errors.email ? <small className="field-error" id="email-error">{errors.email}</small> : null}</label>
        <label><span>Phone number (optional)</span><input name="phone" type="tel" autoComplete="tel" inputMode="tel" value={form.phone} onChange={(e) => update("phone", e.target.value)} aria-invalid={Boolean(errors.phone)} aria-describedby={errors.phone ? "phone-error" : undefined} />{errors.phone ? <small className="field-error" id="phone-error">{errors.phone}</small> : null}</label>
      ...
      <section className={`consent-card ${errors.consent ? "invalid" : ""}`}><label><input type="checkbox" name="consent" checked={form.consent} onChange={(e) => update("consent", e.target.checked)} aria-invalid={Boolean(errors.consent)} aria-describedby={errors.consent ? "consent-error" : "consent-copy"} /><span><strong>I consent to this consultation request</strong><small id="consent-copy">{CONSENT_WORDING}</small></span></label>{errors.consent ? <small className="field-error" id="consent-error">{errors.consent}</small> : null}</section>
