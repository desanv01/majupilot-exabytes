...
export function AccountCaseSync() {
    ...
    const save = () => {
      ...
      void saveActiveAccountCase().then(() => setError("")).catch((cause: unknown) => {
        setError(cause instanceof Error && cause.message === "IDEMPOTENCY_CONFLICT"
          ? "This case changed on another device. Open Saved cases to resolve it before continuing."
          : "Your latest edits are in this browser, but could not be saved to your account. Open Saved cases to retry.");
      });
    };
  ...
  return error ? <aside className="account-save-warning" role="alert">{error}</aside> : null;
}
