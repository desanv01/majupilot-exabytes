...
export type AiOperation = "assessment_follow_up" | "recommendation_explanation" | "advisor_review" | "consultant_note_draft" | "transformation_copilot";
...
export function operationPolicy(operation: AiOperation): AiOperationPolicy {
  const suffix = operation === "assessment_follow_up" ? "FOLLOW_UP" : operation === "recommendation_explanation" ? "RECOMMENDATION_EXPLANATION" : operation === "consultant_note_draft" ? "CONSULTANT_NOTE" : operation === "transformation_copilot" ? "COPILOT" : "ADVISOR_REVIEW";
  ...
  return {
    ...
    perCallCostUsd: moneyEnv(`AI_MAX_COST_USD_${suffix}`, operation === "assessment_follow_up" ? 0.02 : operation === "transformation_copilot" ? 0.10 : explanation ? 0.05 : operation === "consultant_note_draft" ? 0.05 : 0.08),
  };
}
