...
export const clearScenarioComparison = (storage: ScenarioStorage) => storage.removeItem(SCENARIO_STORAGE_KEY);
export function saveScenarioComparison(storage: ScenarioStorage, result: ScenarioComparison) { storage.setItem(SCENARIO_STORAGE_KEY, JSON.stringify(scenarioComparisonSchema.parse(result))); }
